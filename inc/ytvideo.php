<script>
	$(document).ready(function() {
		...
		const videoPlayModes = new Array();
		$('[data-video]').each(function() {
			...
			if (this.hasAttribute('data-play-on-load')) {
				videoPlayModes.push(true);
			} else {
				videoPlayModes.push(false);
			}
		});
		...
		for (let i = 0; i < videos.length; i++) {
			...
			$(videos[i]).click(function() {
				...
				if (videoPlayModes[i] == true) {
					videoToInsert.setAttribute('autoplay', '1');
					videoToInsert.setAttribute('allow', 'autoplay');
				}
